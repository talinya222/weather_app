(() => {
    ...
    function formatDay(timestamp) {
        let date = new Date(1e3 * timestamp);
        let day = date.getDay();
        return weekdays[day];
    }
    function displayForecast(response) {
        let forecast = response.data.daily;
        let forecastElement = document.querySelector(".footer__body");
        let forecastHTML = `<div class="footer__items">`;
        forecast.forEach((function(forecastDay, index) {
            if (index < 7 && index > 0) forecastHTML += `\n\t\t\t\t<article class="footer__item item-footer">\n\t\t\t\t\t<a href="" class="item-footer__image-ibg"><img src="http://openweathermap.org/img/wn/${forecastDay.weather[0].icon}.png" alt="${forecastDay.weather[0].main}"></a>\n\t\t\t\t\t<div class="item-footer__temperature">${Math.round(forecastDay.temp.max)}&deg;</div>\n\t\t\t\t\t<div class="item-footer__weekday">${formatDay(forecastDay.dt)}</div>\n\t\t\t\t</article>\n\t\t`;
        }));
        forecastHTML += `</div>`;
